      ...
      <el-table v-loading="loading" :data="jobList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="任务组" prop="jobGroup" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="row.jobGroup === 'DEFAULT' ? '' : 'warning'">
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="800px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="调用目标方法" prop="invokeTarget">
          <el-input v-model="form.invokeTarget" placeholder="Bean调用示例：taskHandler.execute('参数')" />
          <div class="el-form-item__tip">
            <p>Bean调用示例：taskHandler.execute('参数')</p>
            <p>参数说明：支持字符串，布尔类型，整数类型，浮点类型</p>
          </div>
        </el-form-item>
...
import { 
  ...
  type Job, type JobQuery, type JobForm 
} from '@/api/job'
