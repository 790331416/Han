# XuMan Cloud 项目开发规范文档

## 文档说明

本文档是 **XuMan Cloud** 企业级多租户微服务平台的强制性开发规范，所有开发人员必须严格遵守。违反本规范的代码将**不予合并**。

**版本**: 1.0.0  
**更新日期**: 2026-01-29  
**适用范围**: XuMan Cloud 全部代码仓库
...
```java
// ✅ 正确：通过基础工具类使用
import com.xuman.common.core.util.StringUtils;
import com.xuman.common.core.util.DateUtils;
import com.xuman.common.core.util.CollectionUtils;
...
**Hutool 工具包** - 允许在 `xuman-common-core` 模块中引入：

```xml
<!-- ✅ 仅允许在 xuman-common-core 模块引入 -->
<dependency>
...
**使用限制**：
- ✅ 只能在 `xuman-common-core` 模块的工具类中使用
...
XuMan Cloud 采用 **"基类下沉通用逻辑，子类分层定义门面"** 的 A/I/B 模式：
...
import com.xuman.common.security.annotation.PreAuthorize;
...
XuMan Cloud 使用 **PO → DTO → Query** 三级分层：
...
```java
package com.xuman.system.converter;

import com.xuman.system.domain.po.SysUserPo;
import com.xuman.system.domain.dto.SysUserDto;
import org.mapstruct.Mapper;
import org.mapstruct.Mapping;
import org.mapstruct.factory.Mappers;
...
import com.xuman.api.system.domain.UserDto;
import com.xuman.common.core.domain.R;
...
```java
package com.xuman.common.web.config;
...
class HttpExchangeClientRegistrar implements org.springframework.beans.factory.config.BeanFactoryPostProcessor {
    ...
    public void postProcessBeanFactory(
        ...
        // 扫描 com.xuman.api 包下的所有 @HttpExchange 接口
        ...
        for (org.springframework.beans.factory.config.BeanDefinition bd : 
             scanner.findCandidateComponents("com.xuman.api")) {
            ...
            try {
                ...
                if (annotation != null) {
                    // 提取服务名（从路径中推导，如 /system -> xuman-system）
...
```java
package com.xuman.tenant.config;
...
public class HttpClientConfig {
    ...
    public SystemServiceClient systemServiceClient(WebClient.Builder builder) {
        WebClient client = builder
            .baseUrl("http://xuman-system")  // 服务名（自动负载均衡）
            .build();
...
XuMan Cloud **仅允许使用 GET 和 POST 两种 HTTP 方法**：
...
```java
package com.xuman.common.security.constant;
...
| ❌ 直接引入 Hutool | 耐散布在业务代码中 | ✅ 通过 xuman-common-core 封装 |
...
| ❌ 业务代码直接调用 Hutool | 难以统一管理 | 通过 xuman-common-core 工具类 |
...
| 版本号 | 日期 | 变更内容 | 作者 |
|--------|------|---------|------|
| 1.0.1 | 2026-01-29 | 更新 RESTful 路由规范（仅允许 GET/POST）、添加 HttpExchange 自动配置、添加 Hutool 使用规范 | XuMan Team |
| 1.0.0 | 2026-01-29 | 初始版本 | XuMan Team |

---

**© 2026 XuMan Cloud Team. All Rights Reserved.**
