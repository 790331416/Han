...
# AI 短剧（aivideo）的建表与播种目前只有 PostgreSQL 版：`ai_video_*` 用了 BIGSERIAL、
# Prompt 模板用了 $tag$ 美元引用，都要重写才能落到 MySQL；在没有实库验证之前，
# 把它们硬移植过去等于对外宣称一个没验证过的支持面。
#
# 因此双库播种行数比对跳过 aivideo 行，其余每一行仍然严格比对——
# 平台侧漏播（例如 small 曾漏掉 sys.login.wechatEnabled）照样会被拦下。
# aivideo 自身的种子完整性由 han-aivideo 的 AivideoPromptTemplateSqlTest 保证，
# 它直接断言 full-init.sql 里必须含哪些表、菜单和 Prompt 硬规则。
#
# 若将来 aivideo 要支持 MySQL，删掉这个豁免即可让门禁重新覆盖。
AIVIDEO_ROW_RE = re.compile(r"aivideo|ai_video|AI短剧|AIVideo", re.I)
...
def _seed_row_counts(path: Path) -> dict:
    ...
    for stmt in split_sql_statements(strip_sql_comments(path.read_text(encoding="utf-8", errors="replace"))):
        ...
        # 只认 VALUES 子句后面的元组。早期实现扫整条语句里的所有括号，
        # 会把 Prompt 正文里的英文括注（(System Role)、(Lip-sync)）和 DO 块里的
        # 变量元组当成数据行；纯 INSERT ... SELECT 没有 VALUES，本就不该计数。
        # master 侧的 `INSERT ... SELECT ... FROM (VALUES (..),(..)) AS v(..)` 仍能命中。
        values = re.search(r"(?is)\bVALUES\b(.*)", body)
        if not values:
            continue
        tail = re.sub(r"(?is)\)\s*AS\s+\w+\s*\([^)]*\)\s*$", ")", values.group(1).rstrip())
        rows = [r for r in re.findall(r"\((?:[^()']|'[^']*')*\)", tail)
                if not AIVIDEO_ROW_RE.search(r)]
        if rows:
...
def check_seed_row_count_parity(violations: list) -> None:
    ...
    AI 短剧（aivideo）的播种行不参与本比对，理由见 AIVIDEO_ROW_RE 的说明。
    """
    for tier in ("small", "medium", "full"):
