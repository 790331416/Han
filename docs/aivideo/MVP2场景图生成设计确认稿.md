...
| 是否改 SQL | 否 |
| 是否发布部署 | 否 |
| 是否需要 BOSS 再确认 | 是，确认后才进入 MVP2 实现 |
...
- 默认候选数沿用项目参数 `imageCandidateCount`，当前默认 3 张。
- 场景图默认纯场景、无人，优先服务后续图生视频参考图。
- 生成结果写入 `ai_video_media_asset`，选中结果回写 `ai_video_scene.locked_media_id`。
...
火山官方能力依据：
- 火山方舟文档入口包含图片生成 API、图片生成流式响应、Seedream 4.0-5.0 提示词指南、Seedream 助力 Seedance 生视频最佳实践。
...
flowchart TD
  ...
  G --> H["han-ai 调用火山图片模型"]
  H --> I["图片结果归档到 han-file 或保存可访问 URL 快照"]
  I --> J["写入 ai_video_media_asset 候选图"]
  J --> K["前端展示候选图 3 选 1"]
  K --> L["用户确认选中图"]
  L --> M["回写 ai_video_scene.locked_media_id"]
  M --> N["场景图进入后续视频参考图流程"]
```
...
交互要求：
- 未确认的场景也允许预览提示词，但生成图按钮建议禁用或二次提示。
- 已选图后，再次点击生成图应提示“会新增候选，不覆盖已选图”。
- 同一场景生成中时，该场景的生成按钮禁用，防止重复任务。
- 不同场景可并行生成，但第一版可限制为前端单任务，降低并发风险。
...
- 图片模型：展示当前项目 `imageModelId` 对应模型名，只读或下拉二选一需后续确认。
- 图片候选数：默认 3，第一版允许 1 到 4。
- 画幅：沿用项目画幅，默认 `9:16`。
- 清晰度：沿用 `defaultResolution`，当前默认 `720p`。
- 补充提示词：继续复用当前 `customPrompt`，本次生成写入任务快照。
...
1. 固定系统约束：电影级纯净场景、无人、无角色、无人物剪影、无多余主体。
...
| 方案 | 说明 | 建议 |
| --- | --- | --- |
| A | 新增 `scene_image_prompt_template_id` 到 `ai_video_project_setting` | 更清晰，需 SQL 升级 |
| B | 第一版先把模板 ID 放进 `ai_video_project_setting.params_json` | 改动小，后续再补字段 |

推荐先用方案 B，降低 MVP2 首次开发 SQL 风险；如果 BOSS 希望长期结构更清楚，再改方案 A。
...
- 如果火山图片接口是同步返回，则后端转换为 SSE 的 `meta -> candidate -> done` 事件，避免前端普通 POST 超时。
- 如果火山图片接口是异步任务，则 SSE 先返回任务状态，后端轮询或交给后续 `han-job`。
...
| `biz_type` | `SCENE` |
| `biz_id` | `scene_id` |
| `file_id` | 优先关联 `sys_file.id`，如果第一版存储未打通可为空 |
| `file_url` | 图片访问 URL 快照 |
...
第一版推荐：
- 不新增媒体候选表。
- `ai_video_media_asset` 已存在时不改表。
- `ai_model.model_type` 可直接存 `IMAGE`，当前字段无约束。
- 新增默认 Prompt 模板需要进入 `sql/tiers/full/full-init.sql` 和 `sql/upgrades/postgres/*`。
- 如果选择 `scene_image_prompt_template_id` 字段方案，则另行确认 SQL 升级。
...
| `han-file` | 图片归档、文件 URL、缩略图能力；第一版如未打通可保存 URL 快照 |
...
stateDiagram-v2
  ...
  GENERATING --> READY: provider success
  GENERATING --> FAILED: provider failed or timeout
...
- 使用 BOSS 页面测试：对一个场景生成 3 张候选图，选择 1 张，刷新后仍显示已选图。
...
| 图片返回临时 URL 过期 | 页面后续看不到图 | 优先归档到 `han-file`，至少保存 URL 快照并标注过期风险 |
| 图片生成耗时导致超时 | 页面体验差 | 后端统一 SSE，必要时转异步任务 |
| 场景图出现人物 | 不符合纯场景要求 | Prompt 强约束 + 失败后允许重生；后续可接审核 |
| 成本不可控 | 误点多次消耗 | 前端禁用重复点击，后端运行中任务幂等，候选数限制 |
| 媒体资产表字段不够 | 后续扩展受限 | 第一版用 `params_json` 承接供应商扩展字段 |

回滚方式：
- 前端隐藏场景图生成入口。
- 后端保留接口但禁用图片模型调用配置。
- 删除或忽略未选中的 `ai_video_media_asset` 记录，不影响已确认文字资产。
- 若新增 Prompt 模板有问题，可停用模板或回滚升级脚本。

## 15. 待 BOSS 确认项

1. MVP2 第一段是否只做“单场景生成候选图 3 选 1”。
2. 场景图是否默认强制“纯场景、无人”，不允许出现人物。
3. 图片候选数第一版是否限制为 1 到 4，默认 3。
4. 图片模型是否继续通过 Han 的 AI 模型配置页录入，`model_type = IMAGE`。
5. 场景图 Prompt 模板 ID 是否先放 `ai_video_project_setting.params_json`，后续再补独立字段。
6. 图片结果是否优先归档到 `han-file`；若归档暂不通，是否允许第一版保存火山返回 URL 快照。
7. 是否允许我下一步进入 MVP2 场景图生成开发方案拆解，再确认后开始写代码。
