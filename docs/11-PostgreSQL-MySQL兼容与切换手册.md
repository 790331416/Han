...
| AI 短剧（aivideo） | 支持 | **不支持** | 只有 PostgreSQL 建表与播种，见 §1.4 |
...
### 1.4 AI 短剧（aivideo）只支持 PostgreSQL

2026-08-13 把 `aivideo-dev` 合入后，full 档带上了 AI 短剧模块。它的数据层**只有
PostgreSQL 版**：

- 11 张 `ai_video_*` 表只写在 `sql/tiers/full/full-init.sql`，
  `full-init-mysql.sql` 里没有对应建表。
- 字典、Prompt 模板种子与两个管理端菜单（短剧任务监管、短剧基础配置）同样只在
  PostgreSQL 侧播种。
- 29 个 `20260521_aivideo_*` 起的升级脚本全部只在 `sql/upgrades/postgres/`。

没有一并移植到 MySQL 的原因：`ai_video_*` 用了 `BIGSERIAL`，Prompt 模板正文用了
`$tag$` 美元引用，两者都要重写；在没有实库验证之前移植过去，等于对外宣称一个
没验证过的支持面。

因此 **full 档若选用 MySQL，AI 短剧不可用**。`han-aivideo` 的 pom 仍然带了
MySQL 驱动，那只保证进程能起来、不会因缺驱动在 SSL 协商阶段失败，不代表功能可用。

双库播种行数门禁 `check_seed_row_count_parity` 会跳过 aivideo 行（见
`AIVIDEO_ROW_RE` 处的说明），其余每一行仍严格比对；aivideo 自身的种子完整性由
`han-aivideo` 的 `AivideoPromptTemplateSqlTest` 断言。将来要支持 MySQL，
删掉那个豁免即可让门禁重新覆盖。
