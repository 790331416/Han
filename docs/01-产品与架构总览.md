...
- `han-visual/han-monitor`：基于 Spring Boot Admin 的监控中心，默认端口 `9100`
  - 已由根 `pom.xml` 纳入聚合构建，执行 `mvn compile` 时会参与编译
  - `deploy/small`、`deploy/medium`、`deploy/full` 三档 compose 均不包含该服务，也不在第 6 节系统拓扑中，需要单独启动
  - 该模块的去留尚未定论，本节只记录现状，不代表已废弃或已移除

第 2 节能力矩阵与第 3 节能力口径中的「监控」「日志监控」指 `han-system` 提供的运行时监控能力，与 `han-visual/han-monitor` 不是同一套实现。
